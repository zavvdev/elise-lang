...
pub fn print_err(schema_err: &CsvSchemaResolverErr) {
    ...
    let info: String = match schema_err {
        ...
        TooManySchemaDefArgs { pos: _ } => "Schema definition function should have only one argument".to_string(),
        EmptyRow { pos: _ } => "Row definition does not have any column descriptors".to_string(),
        RowInvalArgsLen { pos: _ } => "Arguments length for the row definition function must be even".to_string(),
